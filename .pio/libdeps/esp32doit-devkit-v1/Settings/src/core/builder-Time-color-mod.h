...
class Builder {
   ...
   public:
    ...
    //}
	    ...
	    // ================= TIME =================
    // время [результат - секунды с начала суток], подключаемая переменная - uint32_t
    bool Time(size_t id, Text label = "", uint32_t* value = nullptr, uint32_t color = SETS_DEFAULT_COLOR) {
        _widget(Code::time, id, label, value, color);
        return _isSet(id, value);
    }
    bool Time(Text label = "", uint32_t* value = nullptr, uint32_t color = SETS_DEFAULT_COLOR) {
        return Time(_next(), label, value);
    }
    // bool Time(size_t id, Text label = "", uint32_t* value = nullptr, sets::Colors color) {
    //     _widget(Code::time, id, label, value, (uint32_t)color);
    //     return _isSet(id, value);
    // }
    // bool Time(Text label = "", uint32_t* value = nullptr, sets::Colors color) {
    //     return Time(_next(), label, value);
    // }
